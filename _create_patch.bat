...
set result_file="%patches_dir%\%source_version%.patch"

echo.
echo 创建从%source_version%升级到%target_version%的补丁%result_file%
echo.

hdiffz.exe "再度魔改版史诗装备计算器_%source_version%_by风之凌殇" "再度魔改版史诗装备计算器_%target_version%_by风之凌殇" "%result_file%"
EXIT /B %ERRORLEVEL%
