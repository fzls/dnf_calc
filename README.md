...
7. 根据游戏内描述，将宠物、称号和所有装备的暗抗词条和移速词条填写到对应配置中
8. 默认大恍惚增幅强化数值改为0，改为推荐使用自定义特色来实现不同部位应用不同的增幅数值
9. 配置工具（by天府星）: 编辑框提示随主属性选择变换
10. 配置工具（by天府星）: 切换无配置存档时清空界面
11. 配置工具（by天府星）: 调整界面元素间距策略
12. 配置工具（by天府星）: 修正一个武器名称错误
13. 配置工具（by天府星）: 百分比属性添加 % 标识
14. 增加配置是否考虑手动切装（指登记和身上不同神话）的方案
15. 更新默认自定义特色示例
16. 更新奶妈基础职业、默认打桩系数
17. 称号和宠物中，概率触发的词条均视为百分百触发（因为实战几乎必定触发）
