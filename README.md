...
# 更新公告
1. 扩展奶系切装到任意件
2. 切装取消【神话只切神话，ss只切ss】的限制
3. 增加换装槽位补正功能，详见自定义配置文件开头
4. 增加配置剪枝过程中允许比当前最大值小的最大差值，从而将一些套装词条数较小的也加入搜索范围
5. base_job_passive_lv15_taiyang原来默认的三级被动经colg网友确认是称号1级（lv15-20的太阳称号）和白金2级，所以可以干掉了，可以自行在自定义配置中添加（若祝福换装与太阳换装不一样，记得补正）
6. 默认配置中，由于太阳额外一觉buff去掉了，需要修正初始智力和体精
7. 其他改动及上述功能具体用法，详见README.md
