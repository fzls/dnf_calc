...
2. 主界面增加tips
3. 一键点亮全部、重置可配置是否包含神话、武器
4. 重构ui布局逻辑，改为配置驱动，并支持嵌套打开多层次窗口，简化后续添加新装备的流程，可参考示例自行调整布局和层次结构
5. 基于新版ui布局框架来重新实现界面布局，以及实现与装备、套装的点击、检查、一件点亮、重置等逻辑
6. 修正一个可能因时序导致重置装备时，神话装备仍在闪烁的问题
7. 增加配置是否在启动时打开特定嵌套区域
8. 新增黄字和爆伤词条相关词条、数据、逻辑、周边处理等
9. 从韩服同步一些数据，修正部分装备的属性
10 同步新的智慧产物、轮回、时空系列装备、超界板甲、普雷装备，并修正相关数据.
11. 添加新装备对应图标、套装图标等各种数据，以及处理新装备的一些特殊逻辑
12. 基于新ui布局逻辑来添加新装备到界面上
13. 调整装备存盘的实现，从而有添加新装备的时候能自动加入到preset中
14. 扩展可供存档的装备数目为1300
15. 解决扣减技攻时算法有问题的bug
16. 计算技攻时使用最终词条值
17. 调整武器的判定方式，改为使用通用规则判定
18. 增加是否启用负数的装备补正配置和是否启用换装槽位补正配置
19. 其他改动及上述功能具体用法，详见README.md
